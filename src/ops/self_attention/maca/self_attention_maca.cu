#include "self_attention_maca.hpp"
...
void self_attention_impl(T *attn_val, const T *q, const T *k, const T *v,
    ...
    if (mcMalloc(reinterpret_cast<void **>(&scores),
                 qlen * nh * kvlen * sizeof(float)) != mcSuccess) {
        throw std::runtime_error("mcMalloc(scores) failed");
    }
...
void self_attention(std::byte *attn_val, const std::byte *q, const std::byte *k, const std::byte *v,
    ...
    case LLAISYS_DTYPE_BF16:
        self_attention_impl(reinterpret_cast<__maca_bfloat16 *>(attn_val),
